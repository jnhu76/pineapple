...
std::ostream&
operator<<(std::ostream& os, TokenType token_type)
{
  switch (token_type) {
...
std::map<std::string, TokenType> KEYWORDS{ { "print",
...
bool
Lexer::next_source_code_is(const std::string& prefix)
{
...
std::string
Lexer::scan_pattern(const std::regex pattern)
{
...
std::string
Lexer::scan_before_token(const std::string& token)
{
  auto str = source_code.substr(current);
...
void
Lexer::process_new_line(const std::string& ignored)
{
  std::size_t i{ 0 };
...
TokenInfo
Lexer::next_token_is(TokenType guess)
{
  std::string token;
